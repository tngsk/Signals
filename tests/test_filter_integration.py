...
class TestFilterIntegration:
    ...
    def test_filter_types_comparison(self):
        ...
        with SynthContext(sample_rate=48000):
            ...
            osc.set_parameter("frequency", 1000.0)
            ...
            # Configure all filters with same parameters
            for filt in filters.values():
                filt.set_parameter("cutoff_frequency", 1000.0)
                filt.set_parameter("resonance", 2.0)
            
            # Process same signal through all filters
            ...
            # Validate that different filter types produce different results
            lowpass_rms = np.sqrt(np.mean([x**2 for x in results['lowpass']]))
            highpass_rms = np.sqrt(np.mean([x**2 for x in results['highpass']]))
            bandpass_rms = np.sqrt(np.mean([x**2 for x in results['bandpass']]))
            
            # Different filter types should produce different RMS levels
            assert lowpass_rms != highpass_rms
            assert lowpass_rms != bandpass_rms
            assert highpass_rms != bandpass_rms
